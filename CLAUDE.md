...
- **YearConfig** (per session, per year): number of 4-yr sections + number of 6-yr sections + a **separate strength per stream** (`strength_4yr`, `strength_6yr`) — the two B.Tech streams often have different section sizes. The legacy single `strength_per_section` is still accepted on input and backfilled into both via a `model_validator` (see `_coerce_year_strengths`) so old docs/clients keep working. Saving a YearConfig **auto-generates Sections** in the DB — frontend never POSTs sections directly. Labels: a single 4-yr section is `{y}/4 CSE`; multiple 4-yr get `{y}/4 CSE - N`; 6-yr always numbered `{y}/6 CSE - N`.
